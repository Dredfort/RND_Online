...
DEFINE_LOG_CATEGORY_STATIC(RNDGameInstanceLog, Log, All);
...
void UNWGameInstance::FindSessions(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, bool bIsLAN, bool bIsPresence)
{
	...
	else
	{
		...
		UE_LOG(RNDGameInstanceLog, Log, TEXT("Starting to find session FAILED!"));
	}
}
...
{
	UE_LOG(RNDGameInstanceLog, Log, TEXT("Try to join to the session: [%s]"), *SessionName.ToString());
	...
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid() && UserId.IsValid())
		{
			...
			UE_LOG(RNDGameInstanceLog, Log, TEXT("Join to [%s] is [%s]"), *SessionName.ToString(), bSuccessful == true ? TEXT("success") : TEXT("failed"));
		}
	}
...
{
	UE_LOG(RNDGameInstanceLog, Log, TEXT("Try join to [%s]"), *SessionName.ToString());
	...
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid())
		{
			...
			{
				UE_LOG(RNDGameInstanceLog, Log, TEXT("Starting travel to [%s]"), *SessionName.ToString());
				UE_LOG(RNDGameInstanceLog, Log, TEXT("TravelURL: [%s]"), *TravelURL);
